...
3. Configure docker-compose.yml File (Copy docker-compose.template.yml)
...
4. Run jackson-load-balancer

NOTE: Make sure you have Docker Desktop or a Docker daemon running on your host machine.

```
$ docker compose up -d
```

### Make HTTP Requests To Your API Pool

Currently, jackson-load-balancer only has support for the following request methods:
...
All requests should only be HTTP as there are no SSL/TLS certificates setup. However, your destination URLs can be either HTTP or HTTPS. NOTE: Make sure to include http:// or https:// in the URL string in the API Pool.

a. Make a health check using the default health check route

```
$ curl http://localhost:5555/api/jackson-health-check
$ curl http://localhost:5555/api/jackson-health-check --request GET # default
```

b. Or, make a request to any routes on your destination API(s)

```
$ curl http://localhost:5555/my/favorite/route/to/add/data --request POST --data '{}'
$ curl http://localhost:5555/my/favorite/route/to/delete/data --request DELETE --data '{}'
$ curl http://localhost:5555/my/favorite/route/to/update/data --request PUT --data '{}'
```

### API Pool Requests

Get Current API Pool

```
$ curl http://localhost:5555/api/jackson-api-pool --request GET
```

Add URL To API Pool

```
$ curl http://localhost:5555/api/jackson-api-pool --request POST --data '{"url":"abc@xyz.com"}'
```

Delete URL From API Pool

```
$ curl http://localhost:5555/api/jackson-api-pool --request DELETE --data '{"url":"abc@xyz.com"}'
```

Replace Entire API Pool
